...
source scripts/bin/queryPackagedComponent.sh authToken=$authToken componentId=$componentId packageVersion=$packageVersion componentType=$componentType
if [ -z "$packageId" ]
then
...
then
	...
	source scripts/bin/queryDeployedPackage.sh envId=$envId packageId=$packageId
...
then
	echo Checking code!...
        source scripts/initCheckRules.sh" packageId=$savePackageId componentId=$saveComponentId extractComponentXmlFolder=CodeReview
fi          
